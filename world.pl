...
rechts(X, Y, U, V) :- kamer(X, Y), kamer(U, V), Y = V, U is X + 1.
links(X, Y, U, V) :- kamer(X, Y), kamer(U, V), Y = V, U is X - 1.
boven(X, Y, U, V) :- kamer(X, Y), kamer(U, V), Y = V - 1, X = U.
onder(X, Y, U, V) :- kamer(X, Y), kamer(U, V), Y = V + 1, X = U.
